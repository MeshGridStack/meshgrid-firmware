/**
 * meshgrid MQTT Bridge - Optional MQTT support for ROOM mode
 *
 * Bridges mesh messages to/from MQTT broker when WiFi is available.
 * Enable with -DMESHGRID_MQTT_SUPPORT build flag.
 */
 ...
 */
typedef void (*mqtt_msg_cb)(const char *topic, const uint8_t *payload,
