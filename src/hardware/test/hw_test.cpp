...
int hw_test_battery(struct hw_test_result* result, uint32_t duration_ms, hw_test_progress_cb progress) {
    ...
    if (progress)
        progress("Reading initial battery...", 0);
    ...
    if (result->battery_start_mv == 0) {
        if (progress)
            progress("No battery detected", 100);
        return -1;
    }
    ...
    if (progress)
        progress("Battery test complete", 100);
...
int hw_test_solar(struct hw_test_result* result, hw_test_progress_cb progress) {
    ...
    if (progress)
        progress("Testing solar input...", 0);
...
int hw_test_radio(struct hw_test_result* result, hw_test_progress_cb progress) {
    ...
    if (!get_radio()) {
        if (progress)
            progress("Radio not initialized", 100);
        return -1;
    }

    if (progress)
        progress("Testing radio TX...", 0);
    ...
    if (progress) {
        char status[32];
        snprintf(status, sizeof(status), "TX: %d/%d packets", result->packets_sent, RADIO_TEST_PACKETS);
        progress(status, 100);
    }
...
void hw_test_format_result(char* buf, size_t len, const struct hw_test_result* result, const char* test_name) {
    if (strcmp(test_name, "battery") == 0) {
        snprintf(buf, len,
                 ...
                 "  Drop:  %dmV\n"
                 "  Est:   ~%dmA",
                 result->passed ? "PASS" : "FAIL", result->battery_start_mv, result->battery_end_mv,
                 result->battery_drop_mv, result->estimated_current_ma);
        ...
        snprintf(buf, len,
                 ...
                 "  Power:   ~%dmW\n"
                 "  Charging: %s",
                 result->passed ? "DETECTED" : "NONE", result->solar_mv, result->solar_ma, result->solar_power_mw,
        ...
        snprintf(buf, len,
                 ...
                 "  Packets:  %d/%d\n"
                 "  Duration: %dms",
                 result->passed ? "PASS" : "FAIL", result->tx_power_dbm, result->packets_sent, RADIO_TEST_PACKETS,
                 result->duration_ms);
    } else {
