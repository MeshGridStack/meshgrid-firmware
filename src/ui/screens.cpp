...
void draw_screen_radio(Adafruit_SSD1306* display) {
    ...
    snprintf(line, sizeof(line), "BW:%.1fkHz SF:%d", radio_config.bandwidth, radio_config.spreading_factor);
    display->setCursor(0, y);
    display->print(line);
    y += UI_SPACING_NORMAL;

    /* Coding rate and preamble */
    snprintf(line, sizeof(line), "CR:4/%d Pre:%d", radio_config.coding_rate, radio_config.preamble_len);
    display->setCursor(0, y);
    display->print(line);
...
void draw_screen_security(Adafruit_SSD1306* display) {
    ...
    /* Instructions */
    display->setCursor(0, y);
    display->print("Connect: AUTH <pin>");
