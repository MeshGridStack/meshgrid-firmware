...
extern bool monitor_mode, log_enabled;
    ...
    if (shared_secret != nullptr) {
        ...
        if (dec_len > 0) {
            ...
            if (sender && sender->pubkey[0] != 0 && text_offset == 5) {
                ...
                SerialOutput.print("DEBUG ACK calc: data_len=");
                SerialOutput.print(data_len);
                SerialOutput.print(" text_len=");
                SerialOutput.print(text_len);
                SerialOutput.print(" msg=[");
                for (size_t i = 0; i < min(data_len, (size_t)20); i++) {
                    if (i > 0) SerialOutput.print(" ");
                    SerialOutput.print("0x");
                    if (decrypted[i] < 16) SerialOutput.print("0");
                    SerialOutput.print(decrypted[i], HEX);
                }
                Serial.println("]");
    ...
    switch (pkt.payload_type) {
            ...
            if (pkt.route_type == ROUTE_DIRECT && pkt.path_len < MESHGRID_MAX_PATH_SIZE) {
                ...
                if (offset >= len) {
                    ...
                    if (tx_len > 0) {
                        ...
                        String log_msg = "TRACE dest reached (hops: ";
                        log_msg += String(pkt.path_len);
                        log_msg += ")";
                        log_event(log_msg);
                    }
        ...
        case PAYLOAD_PATH:
            ...
            if (pkt.payload_len >= 5) {
                ...
                /* Output JSON with path information */
                Serial.print("{\"type\":\"trace_response\",\"trace_id\":");
                ...
                for (int i = 0; i < hop_count && i < 32; i++) {
                    if (i > 0) Serial.print(",");
                    Serial.print("\"0x");
                    Serial.print(pkt.payload[5 + i], HEX);
                    Serial.print("\"");
                }
                ...
                String log_msg = "TRACE response: ";
                log_msg += String(hop_count);
                log_msg += " hops";
                log_event(log_msg);
            }
            return;
...
void send_advertisement(uint8_t route_type) {
    ...
    if (tx_len > 0) {
        SerialOutput.print("TX ADV: total_len=");
        SerialOutput.print(tx_len);
        SerialOutput.print(" payload_len=");
        SerialOutput.print(pkt.payload_len);
        SerialOutput.print(" sig_valid=");
...
void send_text_message(uint8_t dest_hash, const char *text) {
    ...
    if (tx_len > 0) {
        ...
        SerialOutput.print("DEBUG TX type=");
        SerialOutput.print(pkt.payload_type);
        SerialOutput.print(" dest=0x");
        SerialOutput.print(dest_hash, HEX);
        SerialOutput.print(" src=0x");
        SerialOutput.print(mesh.our_hash, HEX);
        SerialOutput.print(" payload_len=");
        SerialOutput.print(pkt.payload_len);
        SerialOutput.print(" header=0x");
        SerialOutput.print(pkt.header, HEX);
        SerialOutput.print(" payload: ");
        for (int j = 0; j < min(12, (int)pkt.payload_len); j++) {
            SerialOutput.print("0x");
            SerialOutput.print(pkt.payload[j], HEX);
            SerialOutput.print(" ");
        }
        Serial.println();
...
void send_channel_message(uint8_t channel_hash, const uint8_t *channel_secret, const char *text, const char *channel_name) {
    ...
    SerialOutput.print("DEBUG PLAINTEXT len=");
    SerialOutput.print(i);
    SerialOutput.print(" bytes: ");
    for (int j = 0; j < min(i, 64); j++) {
        if (j > 0) SerialOutput.print(" ");
        SerialOutput.print("0x");
        if (plaintext[j] < 16) SerialOutput.print("0");
        SerialOutput.print(plaintext[j], HEX);
    }
    Serial.println();
    ...
    if (tx_len > 0) {
        /* DEBUG: Print packet details */
        SerialOutput.print("DEBUG TX GRP type=");
        SerialOutput.print(pkt.payload_type);
        SerialOutput.print(" channel=0x");
        SerialOutput.print(channel_hash, HEX);
        SerialOutput.print(" payload_len=");
        SerialOutput.print(pkt.payload_len);
        SerialOutput.print(" enc_len=");
        SerialOutput.print(enc_len);
        SerialOutput.print(" header=0x");
        SerialOutput.print(pkt.header, HEX);
        SerialOutput.print(" payload: ");
        for (int j = 0; j < min(12, (int)pkt.payload_len); j++) {
            if (j > 0) SerialOutput.print(" ");
            SerialOutput.print("0x");
            if (pkt.payload[j] < 16) SerialOutput.print("0");
            SerialOutput.print(pkt.payload[j], HEX);
        }
        Serial.println();
