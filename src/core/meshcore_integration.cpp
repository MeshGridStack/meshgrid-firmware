...
namespace MeshCoreIntegration {
    ...
    MeshgridCallbacks callbacks = {
        .get_shared_secret = callback_get_shared_secret,
    ...
    void callback_update_neighbor(const uint8_t* pubkey, const char* name,
                                 uint32_t timestamp, int16_t rssi, int8_t snr,
                                 uint8_t hops, uint8_t protocol_version) {
        DEBUG_INFOF("[MeshCore] callback_update_neighbor: name=%s, rssi=%d, snr=%d, hops=%d",
                    name, rssi, snr, hops);
        neighbor_update(pubkey, name, timestamp, rssi, snr, hops, protocol_version);
    }

    void callback_store_direct_message(const char* sender_name, uint8_t sender_hash,
    ...
    void callback_store_channel_message(uint8_t channel_hash, const char* sender_name,
                                       const char* text, uint32_t timestamp) {
        ...
        if (channel_hash == public_channel_hash) {
            ...
            if (channel_idx >= 0) {
                ...
                if (channel_msg_count[channel_idx] < CHANNEL_MESSAGE_BUFFER_SIZE) channel_msg_count[channel_idx]++;
    ...
    int callback_find_channel_by_hash(uint8_t hash, mesh::GroupChannel channels[], int max_matches) {
        ...
        for (int i = 0; i < custom_channel_count && found < max_matches; i++) {
            if (custom_channels[i].valid && custom_channels[i].hash == hash) {
                ...
                DEBUG_INFOF("[MeshCore] callback_find_channel_by_hash: found CUSTOM channel '%s'",
    ...
    void initialize() {
        ...
        mesh_v0 = new MeshgridMesh(*radio_adapter, *clock_adapter, *rng_adapter,
                                   *rtc_adapter, *packet_manager, *tables_adapter,
                                   &callbacks, radio_adapter);
        ...
        DEBUG_INFOF("MeshCore v0 initialized: name=%s, hash=0x%02x",
                    mesh_get_name(), mesh_get_privkey()[0]);
    ...
    void handle_received_packet(uint8_t* buf, int len, int16_t rssi, int8_t snr) {
        if (!mesh_v0 || !radio_adapter) return;
        ...
        DEBUG_INFOF("[MeshCore] RX packet: len=%d, type=0x%02x, rssi=%d, snr=%d",
                    len, pkt->getPayloadType(), rssi, snr);
