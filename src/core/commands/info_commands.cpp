...
void cmd_stats() {
    ...
    response_print("\"board\":\""); response_print(board->vendor); response_print(" "); response_print(board->name); response_print("\",");
    ...
    response_print("\"heap_free_kb\":"); response_print(telemetry.free_heap / 1024); response_print(",");
    response_print("\"flash_total_kb\":3264,");
    response_print("\"flash_used_kb\":481");
    response_print("},");
    response_print("\"packets\":{");
    response_print("\"rx\":"); response_print(mesh.packets_rx); response_print(",");
    response_print("\"tx\":"); response_print(mesh.packets_tx); response_print(",");
    response_print("\"fwd\":"); response_print(mesh.packets_fwd); response_print(",");
    response_print("\"dropped\":"); response_print(mesh.packets_dropped); response_print(",");
    response_print("\"duplicates\":"); response_print(stat_duplicates);
    response_print("},");
    response_print("\"neighbors\":{");
    response_print("\"total\":"); response_print(neighbor_count); response_print(",");
    response_print("\"clients\":"); response_print(stat_clients); response_print(",");
    response_print("\"repeaters\":"); response_print(stat_repeaters); response_print(",");
    response_print("\"rooms\":"); response_print(stat_rooms);
    response_print("},");
    response_print("\"radio\":{");
    response_print("\"freq_mhz\":"); response_print(radio_config.frequency, 2); response_print(",");
    response_print("\"bandwidth_khz\":"); response_print(radio_config.bandwidth, 1); response_print(",");
    response_print("\"spreading_factor\":"); response_print(radio_config.spreading_factor); response_print(",");
    response_print("\"coding_rate\":"); response_print(radio_config.coding_rate); response_print(",");
    response_print("\"tx_power_dbm\":"); response_print(radio_config.tx_power); response_print(",");
    response_print("\"tcxo_voltage\":"); response_print(board->lora_defaults.tcxo_voltage, 1); response_print(",");
    response_print("\"dio2_as_rf_switch\":"); response_print(board->lora_defaults.dio2_as_rf_switch ? "true" : "false"); response_print(",");
    response_print("\"isr_count\":"); response_print(isr_trigger_count);
    response_print("},");
    response_print("\"power\":{");
    response_print("\"battery_mv\":"); response_print(telemetry.battery_mv); response_print(",");
    response_print("\"battery_pct\":"); response_print(telemetry.battery_pct); response_print(",");
    response_print("\"charging\":"); response_print(telemetry.is_charging ? "true" : "false"); response_print(",");
    response_print("\"usb_power\":"); response_print(telemetry.is_usb_power ? "true" : "false"); response_print(",");
    response_print("\"sleep_enabled\":"); response_print(!telemetry.is_usb_power ? "true" : "false");
    response_print("},");
    ...
    response_print("\"version\":\""); response_print(MESHGRID_VERSION); response_print("\",");
    response_print("\"build\":\""); response_print(MESHGRID_BUILD_DATE); response_print("\",");
    response_print("\"uptime_secs\":"); response_print(get_uptime_secs()); response_print(",");
    response_print("\"mode\":\""); response_print(device_mode == MODE_REPEATER ? "REPEATER" : "CLIENT"); response_print("\"");
    response_print("},");
    response_print("\"temperature\":{");
    if (telemetry.has_temp) {
        response_print("\"cpu_c\":"); response_print(telemetry.temp_deci_c / 10.0, 1);
    } else {
